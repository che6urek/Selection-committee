...
import com.by.evgeny.selection.committee.parser.EnrolleSaxParser;
...
public class ShowEnrolleesCommand implements Command {
    ...
    public Page execute(HttpServletRequest request) {
        ...
        try (InputStream input = new FileInputStream("src/main/resources/conf.properties")) {
            ...
            EnrolleSaxParser enrolleParser = new EnrolleSaxParser();
            XMLReader xmlReader = XMLReaderFactory.createXMLReader();
            xmlReader.setContentHandler(enrolleParser);
            xmlReader.parse(new InputSource(xmlDataFileName));
            enrolleParser.getEnrollees();
        } catch (IOException | SAXException e) {
            logger.error(e.getMessage());
        }

        int rows = enrollees.size();

        if (currentPage * recordsPerPage >= rows) {
            request.setAttribute("enrollees", enrollees.subList((currentPage - 1) * recordsPerPage, rows));
        } else {
            request.setAttribute("enrollees", enrollees.subList((currentPage - 1) * recordsPerPage, currentPage * recordsPerPage));
        }
        ...
        return new Page(Page.ENROLLEES_PAGE_PATH, false);
    }
}
